#!/usr/bin/env python3
"""Encapsulated GitHub repository search and reading for Reach Skill."""
...
def parse_github_target(value: str) -> GitHubTarget:
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        raise GhError("Only github.com and raw.githubusercontent.com URLs are supported by the GitHub wrapper")
...
def command_view(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "input": args.repo,
        "result": run_gh_json(
            ["repo", "view", target.repo, "--json", ",".join(VIEW_FIELDS)],
            args.timeout,
        ),
    }
...
def command_inspect(args: argparse.Namespace) -> dict[str, Any]:
    target = parse_github_target(args.repo)
    args = argparse.Namespace(repo=target.repo, ref=args.ref or target.ref, timeout=args.timeout)
    view_payload = command_view(args)
    dir_payload = command_read_dir(argparse.Namespace(repo=args.repo, path="", ref=args.ref, timeout=args.timeout))

    files: dict[str, Any] = {}
    for path in COMMON_FILES:
        payload = try_read_file(args.repo, path, args.ref, args.timeout)
...
def command_auto(args: argparse.Namespace) -> dict[str, Any]:
    try:
        ...
        payload["route_reason"] = "Input was not a GitHub URL or OWNER/REPO slug, so Reach searched repositories."
        return payload

    if target.kind == "file":
        payload = command_read_file(
            argparse.Namespace(repo=args.input, path="", ref=args.ref, timeout=args.timeout)
        )
        payload["routed_from"] = "auto"
        payload["route_reason"] = "Input was a GitHub file URL."
        return payload
    if target.kind == "dir":
        payload = command_read_dir(
            argparse.Namespace(repo=args.input, path="", ref=args.ref, timeout=args.timeout)
        )
        payload["routed_from"] = "auto"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reach Skill GitHub search and reading wrapper.")
    parser.add_argument(
        "--pretty",
        action="store_true",
        default=argparse.SUPPRESS,
    ...
    read_dir_parser.add_argument("repo", help="Repository in OWNER/REPO format.")
    ...
    read_file_parser.add_argument("repo", help="Repository in OWNER/REPO format.")
    ...
    inspect_parser.add_argument("repo", help="Repository in OWNER/REPO format.")
